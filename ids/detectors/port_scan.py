"""
TCP Port Scan Threat Detector (Phase 6).

Detects horizontal and vertical TCP port reconnaissance by monitoring unique destination
ports probed per source IP within a memory-bounded sliding time window.
"""
...
class PortScanDetector(BaseDetector):
    """
    Detects TCP SYN port scanning patterns.
    """

    def __init__(self, config: Optional[Dict[str, Any]] = None):
        super().__init__(name="PortScanDetector", config=config)
    ...
    def process(self, packet: ParsedPacket) -> Optional[SecurityEvent]:
        """
        Inspect a normalized packet for TCP port scan activity.

        Triggers an alert when a single source IP contacts at least
        `unique_ports_threshold` distinct TCP destination ports within `window_seconds`.
        """
        if not self.enabled or not packet:
            return None

        # We specifically look for TCP connection attempts / SYN probes
        if not (packet.ip and packet.tcp):
            return None

        # Filter for SYN packets without ACK (standard scan signature)
        ...
        if len(active_ports) >= self.unique_ports_threshold:
            ...
            if timestamp - last_alert >= self.alert_cooldown:
                ...
                return SecurityEvent(
                    ...
                    detector=self.name,
                    description=(
                        f"TCP Port Scan detected from {src_ip}: {len(active_ports)} "
    ...
    def reset_state(self) -> None:
        """Clear all active scan trackers."""
        self.trackers.clear()
